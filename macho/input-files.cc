...
void ObjectFile<E>::parse(Context<E> &ctx) {
  ...
  {
    Timer t(ctx, "assign_input_section_subranges_object", &timer);
...
template <typename E>
bool ObjectFile<E>::finish_parse_now(Context<E> &ctx) {
  bool started = false;
  std::call_once(finish_parse_once, [&] {
    if (!fully_parsed) {
      started = true;
      finish_parse(ctx);
    }
...
template <typename E>
bool ObjectFile<E>::start_finish_parse(Context<E> &ctx) {
  bool started = false;
  std::call_once(finish_parse_once, [&] {
    if (fully_parsed)
      return;
    ...
    if constexpr (E::cputype == CPU_TYPE_X86_64) {
      if (ctx.use_archive_finish_early_lane() && can_start_finish_parse_early()) {
        static Counter early_started("num_archive_members_finish_parse_started_early");
        early_started++;
        ctx.enqueue_archive_finish_early([this, &ctx] { finish_parse(ctx); });
        return;
      }
    }

    ctx.tg.run([this, &ctx] { finish_parse(ctx); });
  });
  return started;
}

template <typename E>
bool ObjectFile<E>::can_start_finish_parse_early() const {
  return !this->archive_name.empty() && !may_have_common_symbols;
}
  ...
  for (i64 i = 0; i < this->syms.size(); i++) {
    ...
    if (get_rank(this, msym.is_common(), is_weak) < get_rank(sym)) {
      ...
      case N_ABS:
        ...
        break;
      case N_SECT:
        // Some symbols (notably in always-split sections such as __cstring)
        // don't have a direct sym_to_subsec entry, so fall back to address lookup.
        sym.subsec = sym_to_subsec[i];
        if (!sym.subsec)
          sym.subsec = find_subsection(ctx, msym.value);
        if (!sym.subsec)
...
DylibFile<E>::DylibFile(Context<E> &ctx, MappedFile<Context<E>> *mf)
  ...
  if (ctx.reader.implicit) {
    // Libraries implicitly specified by LC_LINKER_OPTION are dead-strippable
    // unless the linker option explicitly marks them as needed.
    this->is_alive = ctx.reader.needed;
  } else {
