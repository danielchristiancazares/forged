...
public:
  ...
  void clear_symbols();
  void prepare_for_parse() {
    std::scoped_lock lock(parse_mu);
    parse_complete = false;
  }

  void wait_for_parse() {
    std::unique_lock lock(parse_mu);
    parse_cv.wait(lock, [&] { return parse_complete; });
  }
...
public:
  ...
  bool finish_parse_now(Context<E> &ctx);
  bool start_finish_parse(Context<E> &ctx);
...
struct Context {
  ...
  i64 get_archive_finish_lane_width() const {
    if constexpr (E::cputype != CPU_TYPE_X86_64)
      return 0;
    return std::max<i64>(1, std::min<i64>(2, arg.thread_count / 4));
  }

  void init_archive_finish_early_lane() {
    if constexpr (E::cputype != CPU_TYPE_X86_64)
      return;
    archive_finish_early_arena.initialize((int)get_archive_finish_lane_width());
    archive_finish_early_enabled = true;
  }

  bool use_archive_finish_early_lane() const {
    if constexpr (E::cputype != CPU_TYPE_X86_64)
      return false;
    return archive_finish_early_enabled;
  }

  template <typename F>
  void enqueue_archive_finish_early(F fn) {
    ASSERT(use_archive_finish_early_lane());
    archive_finish_early_arena.execute([&] {
      archive_finish_early_tg.run(std::move(fn));
    });
  }

  void wait_archive_finish_early() {
    if (!use_archive_finish_early_lane())
      return;
  ...
  tbb::task_group tg;
  bool archive_finish_early_enabled = false;
  tbb::task_arena archive_finish_early_arena;
